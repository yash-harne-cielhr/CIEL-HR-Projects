...
dotenv.config({ path: path.resolve(__dirname, '..', '..', '.env.internal') });
...
/**
 * Maps each Location Audit role to its login credentials for INTERNAL.
 * Values come from .env.internal — nothing sensitive lives in this file.
 * Tests never import this directly; they go through UserManager.
 */
export const internalUsers: Record<Role, Credentials> = {
  [Role.VENDOR_USER]: {
